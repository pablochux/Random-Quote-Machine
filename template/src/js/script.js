...
function getTrueRandom(arr){
	...
	console.log("randomNum" + randomNum + " arr.length: " + arr.length);
	var randomElement = arr.splice(randomNum, 1);
...
function setRandomColor(color) {
	$("body, .new-quote-container").css("background-color", color);
	$(".quote-container>div>p, .btn-container a i").css("color", color);
}
function main(randomQuotes, backgroundColors){
	setRandomQuote(getRandom(randomQuotes));
...
$(document).ready(function(){
    ...
    // Change the quote and the background when the button .click is clicked
	$(".new-quote").click(function(){
		main(randomQuotes, backgroundColors);
	});
    // Change the quote and the background when the key enter is pressed
    ...
    // Tuit the quote when key t is pressed
    $(document).keydown(function(event) {
		// keypress 13 -> Enter key
		if (event.which == 84){
			window.location = $('.twitter-link').attr('href');
		}
	});
});
